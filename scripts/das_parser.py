...
def annotate_das_class(df: pd.DataFrame, event_type: str, software: str) -> pd.DataFrame:
    """
    Annotate DAS class (up-regulate, down-regulate, non-DAS) based on software-specific criteria.
    """
    df = df.copy()
    ...
    if 'rmats' in software.lower():
        ...
        df['class'] = np.select(conditions, choices, default='non-DAS')
        ...
        for uid, group in df.groupby('uniform_ID'):
            ...
            if group_inclusion.empty:
                df.loc[df['uniform_ID'] == uid, 'class'] = 'non-DAS'
                continue
...
def ase_extract(df: pd.DataFrame, software: str, event_type: str, sample_name: str, input_dir: str) -> Tuple[Set[str], Set[str]]:
    ...
    if 'rmats' in software.lower():
        ...
        # similar filtering as in annotate_das_class
        if event_type == "SE":
